...
class Bank:
    ...
    def balanceequiry(self):
        temp = db_query(
            f"SELECT balance FROM customer2 WHERE username = '{self.__username}';")
    ...
    def deposit(self, amount):
        ...
        db_query(
            f"UPDATE customer2 SET balance = '{test}' WHERE username = '{self.__username}'; ")
        self.balanceequiry()
    ...
    def fundtransfer(self, receive, amount):
        ...
        if amount > temp[0][0]:
            ...
            temp2 = db_query(
                f"SELECT balance FROM customer2 WHERE account_number = '{receive}';")
            if temp2 == []:
                ...
                db_query(
                    f"UPDATE customer2 SET balance = '{test1}' WHERE username = '{self.__username}'; ")
                db_query(
                    f"UPDATE customer2 SET balance = '{test2}' WHERE account_number = '{receive}'; ")
                receiver_username = db_query(
                    f"SELECT username FROM customer2 where account_number = '{receive}';")
                self.balanceequiry()
